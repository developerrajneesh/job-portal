...
export const creteSavedJobs = async (req, res, next) => {
  ...
  try {
    ...
    if(existJobsId){
      return res.status(200) .json({ success: false, msg: "You have already saved for this job" });
    }
...
export const createApplyJobs = async (req, res, next) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ user: user, success: true, msg: "Job Applied saved successfully" });
  } catch (err) {
    next(err);
    console.log(err);
  }
